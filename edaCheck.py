...
def main(project, now, username):
    ...
    OUTPUT_PATH = Path(r'H:\Shared drives\Hevra - Roy Salomon Lab - HIPAA\SafeHeart_research_(NovaHelp)\Experiment\Processed Data\Outputs_tomer')
    DATA_PATH = Path(r'H:\Shared drives\Hevra - Roy Salomon Lab - HIPAA\SafeHeart_research_(NovaHelp)\Experiment\Processed Data\Data_NEW_CODE')
    ...
    SUBJECTS_DATES = METADATA_PATH.joinpath('g:\Shared drives\AdmonPsy - Nova\Experiment\Metadata\Subjects Dates Copy.csv')
    ...
    subjects_list ='C0281 C0331'
    subjects_to_run_on = subjects_list.split(' ')
    # subjects_to_run_on = ['C0239']
    ...
    for subject in tqdm_subjects:
        ...
        subject_dates = get_subject_dates_of_experiment(subject, METADATA_PATH)
        ...
        for x in sessions_df['start_date_time']:
            x_str = str(x)
            if '+' in x_str:
                tz_offset = int(x_str.split('+')[1].split(':')[0])
            elif '-' in x_str:
                tz_offset = -1 * (int(x_str.split('-')[1].split(':')[0]))
            else:
                tz_offset = 0  # Default to 0 if no timezone offset is found
            tz_list.append(tz_offset)

        sessions_df['start_date_time'] = pd.to_datetime(sessions_df['start_date_time'], utc=True)
        ...
        if tz_list != []:
            for i, j in zip(range(len(sessions_df['start_date_time'])), tz_list):
                sessions_df['start_date_time'][i] = sessions_df['start_date_time'][i] + pd.Timedelta(hours=j)
        ...
        for x in sessions_df['end_date_time']:
            x_str = str(x)
            if '+' in x_str:
                tz_offset = int(x_str.split('+')[1].split(':')[0])
            elif '-' in x_str:
                ...
                tz_offset = 0  # Default to 0 if no timezone offset is found
            tz_list_end.append(tz_offset)

        sessions_df['end_date_time'] = pd.to_datetime(sessions_df['end_date_time'], utc=True)
        ...
        if tz_list_end != []:
            for i, j in zip(range(len(sessions_df['end_date_time'])), tz_list_end):
                sessions_df['end_date_time'][i] = sessions_df['end_date_time'][i] + pd.Timedelta(hours=j)
        ...
        mindfulness_output_df = mindfulness_output_df.dropna(subset=['StartMeasureTime'])
...
def get_subject_dates_of_experiment(subject: str, METADATA_PATH) -> pd.DataFrame:
    ...
    subjects_dates_path = Path(r'g:\Shared drives\AdmonPsy - Nova\Experiment\Metadata\Subjects Dates Copy.csv')
