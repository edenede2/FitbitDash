...
def load_outputs(n_clicks, project):
    ...
    return html.Div([
        ...
        dbc.Button('Load', id={
            'type': 'load-sleep-outputs-button',
            'index': 1
        }, n_clicks=0, color='primary'),

        html.Div(id={
            'type': 'sleep-outputs-container',
            'index': 1
        })
    ])

@callback(
    Output({'type': 'sleep-outputs-container', 'index': ALL}, 'children'),
    Input({'type': 'load-sleep-outputs-button', 'index': ALL}, 'n_clicks'),
    State({'type': 'sleep-outputs-dropdown', 'index': ALL}, 'value'),
    State('project-selection-dropdown-FitBit-Sleep-All-Subjects', 'value')
)
def load_output(n_clicks, selected_folders, project):
    ...
    show_button = dbc.Button('Preview selected file', id={
        'type': 'show-preview-button-sleep',
        'index': 1
    }, n_clicks=0, color='primary')
...
@callback(
    Output({'type': 'file-content', 'index': ALL}, 'children'),
    Input({'type': 'show-preview-button-sleep', 'index': ALL}, 'n_clicks'),
    State({'type': 'sleep-files-table', 'index': ALL}, 'selectedRows'),
    State({'type': 'sleep-outputs-dropdown', 'index': ALL}, 'value'),
    State('project-selection-dropdown-FitBit-Sleep-All-Subjects', 'value')
)
